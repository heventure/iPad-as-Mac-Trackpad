...
@MainActor final class iPadPeerSender:NSObject,ObservableObject{
 @Published private(set)var connectedPeerName:String?; @Published private(set)var statusText="正在搜索 Mac…"
 private let serviceType="ipadpad"; private let peerID=MCPeerID(displayName:UIDevice.current.name)
 private lazy var session=MCSession(peer:peerID,securityIdentity:nil,encryptionPreference:.required)
 private lazy var browser=MCNearbyServiceBrowser(peer:peerID,serviceType:serviceType); private var invitedPeers=Set<MCPeerID>()
 override init(){super.init();session.delegate=self;browser.delegate=self;browser.startBrowsingForPeers()}
 deinit{browser.stopBrowsingForPeers();session.disconnect()}
 func send(_ message:PointerMessage){guard !session.connectedPeers.isEmpty,let data=try? JSONEncoder().encode(message) else{return};try? session.send(data,toPeers:session.connectedPeers,with:.unreliable)}
}
