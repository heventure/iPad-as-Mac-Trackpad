...
@MainActor
final class MacPeerReceiver: NSObject, ObservableObject {
...
extension MacPeerReceiver: MCNearbyServiceAdvertiserDelegate {
    ...
    nonisolated func advertiser(_ advertiser: MCNearbyServiceAdvertiser, didNotStartAdvertisingPeer error: Error) {
        Task { @MainActor in self.statusText = "广播失败：\(error.localizedDescription)" }
    }
}
...
extension MacPeerReceiver: MCSessionDelegate {
    ...
    nonisolated func session(_ session: MCSession, didReceive data: Data, fromPeer peerID: MCPeerID) {
        guard let message = try? JSONDecoder().decode(PointerMessage.self, from: data) else { return }
